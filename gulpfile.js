...
gulp.task('copy', ['clean'], function() {
	...
	var images = gulp.src(params.src + 'images/*.*'),
			fancyImgs = gulp.src(params.src + 'images/fancybox/**'),
			fonts = gulp.src(params.src + 'fonts/**');
			ajax = gulp.src(params.src + 'ajax/**');

	images
		.pipe(gulp.dest(params.out + 'images'));
		
	fancyImgs
		.pipe(gulp.dest(params.out + 'images/fancybox'));

	fonts
		.pipe(gulp.dest(params.out + 'fonts'));
		
	ajax
		.pipe(gulp.dest(params.out + 'ajax'));
});
